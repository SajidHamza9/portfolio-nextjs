...
- The **contact form does not send anything** — `components/ContactForm/` uses Formik + Yup (`validation.ts`) and on submit only logs to the console and shows a toast. Wire up a real backend/API if submissions need to go somewhere.
